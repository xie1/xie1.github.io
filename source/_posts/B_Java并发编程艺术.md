...
####2.1、volatile的应用
	1、volatile的定义与实现原理
![Alt text](./1559738867435.png)
![Alt text](./1559738945560.png)
...
![Alt text](./1559740361838.png)
#####2.2.1、Java对象头
	Java对象头里的Mark Word里默认存储对象的HashCode、 分代年龄和锁的标记位。
#####2.2.2、锁的升级与对比--》暂定学习
	锁一共有4中状态，级别从低到高依次：无锁状态，偏向锁状态，轻量级锁状态和重量级锁状态，这几个状态会随着竞争情况逐渐升级
![Alt text](./1559741685921.png)
####2.3、原子操作的实现
![Alt text](./1559742109588.png)

	1、处理器如何实现原子操作
![Alt text](./1559742200401.png)
![Alt text](./1559742233033.png)
![Alt text](./1559742278181.png)
...
#####3.1.1、并发编程模型的两个关键问题
	...
	2、线程之间如何同步
#####3.1.2、Java内存模型的抽象结构
![Alt text](./1559785535632.png)
![Alt text](./1559785554739.png)
![Alt text](./1559785574680.png)
...
![Alt text](./1559791421990.png)
![Alt text](./1559791442442.png)


#####3.3.3、同步程序的顺序一致性效果
![Alt text](./1559791744271.png)
![Alt text](./1559791756980.png)

#####3.3.4、未同步程序的执行特性
![Alt text](./1559792465855.png)
...
![Alt text](./1559793334564.png)
![Alt text](./1559793356601.png)
...
![Alt text](./1559794651189.png)
![Alt text](./1559794704387.png)

#####3.4.4、volatile内存语义的实现——》暂定学习
	为了实现volatile内存语义，JMM会分别限制这两种类型的重排序类型。

![Alt text](./1559795127585.png)
![Alt text](./1559795133582.png)
![Alt text](./1559795255233.png)
![Alt text](./1559795268848.png)
...
#####3.5.2、锁的释放-获取的内存语义
![Alt text](./1559801776698.png)
![Alt text](./1559801803104.png)
![Alt text](./1559801859382.png)
...
#####4.1.3、线程优先级
#####4.1.4、线程状态
![Alt text](./1559807451848.png)
![Alt text](./1559807552916.png)
...
###5、Java中的的锁
####5.1、Lock接口
![Alt text](./1560134641852.png)
![Alt text](./1560134746289.png)
...
![Alt text](./1560135399518.png)

#####5.2.1、队列同步器的接口与示例
	同步器提供的模板方法基本上分为3类：
	1、独占式获取与释放同步状态
	2、共享式获取与释放同步状态和查询同步队列中的等待线程情况
![Alt text](./1560137783664.png)
![Alt text](./1560137794280.png)
![Alt text](./1560137852255.png)
...
![Alt text](./1560138731111.png)
	...
	3、共享式同步状态的获取与释放
![Alt text](./1560140133516.png)
	
	4、独占式超时获取同步状态
![Alt text](./1560140632305.png)
...
####5.5、LockSupport工具
![Alt text](./1560147350158.png)


####5.6、Condition接口
![Alt text](./1560147496166.png)
...
![Alt text](./1560149793754.png)
![Alt text](./1560149810695.png)


#####6.1.3、ConcurrentHashMap的初始化
![Alt text](./1560149959832.png)

	1、初始化segments数组
![Alt text](./1560218201817.png)

	2、初始化segmentShift和segmentMask
![Alt text](./1560218249135.png)


	3、初始化每个segment
		输入参数initialCapacity是ConcurrentHashMap的初始化容量，loadfactor是每个segment的负载因子，在构造方法里需要通过这两个参数来初始化数组总的每个segment
![Alt text](./1560218362849.png)
#####6.1.4、定位Segment
...
![Alt text](./1560220595630.png)

#####6.2.2、入队列

	1、入队列过程
		入队就是将入队节点添加到队列的尾部。
![Alt text](./1560221088390.png)
![Alt text](./1560221099145.png)
![Alt text](./1560221108987.png)
...
#####6.4.3、Fork/Join框架的设计
	1、分割任务
	2、执行任务并合并结果
![Alt text](./1560226045676.png)

#####6.4.4、使用Fork/Join框架
![Alt text](./1560226252020.png)
![Alt text](./1560226264101.png)
...
![Alt text](./1560228291004.png)
#####8.2.2、CyclicBarrier应用场景
	可以用于多线程计算数据，最后合并计算结果的场景
####8.3、控制并发线程数的Semaphore
	Semaphore（信号量）是用来控制同时访问特定资源的线程数量，它通过协调各个线程，以保证合理的使用公共资源
![Alt text](./1560228746728.png)
...
###9、Java中的线程池
	...
	3、提供线程的可管理性
####9.1、线程池的实现原理
![Alt text](./1560234338428.png)
![Alt text](./1560234353450.png)
![Alt text](./1560234388428.png)
![Alt text](./1560234540905.png)
![Alt text](./1560234553917.png)

	工作线程：线程池创建线程时，会将线程封装成工作线程Worker，Worker在执行完任务后，还会循环获取工作队列里的任务来执行。

####9.2、线程池的使用
#####9.2.1、线程池的创建
![Alt text](./1560235094622.png)
![Alt text](./1560235178861.png)

#####9.2.2、向线程池提交任务

	可以使用两个方法向线程池提交任务，分别是execute（）和submit()
	1、execute ：用于提交不需要返回值得任务，所以无法判断任务是否被线程池执行成功
![Alt text](./1560236034369.png)

	2、submit（），用于提交需要返回值的任务。
![Alt text](./1560236098368.png)

#####9.2.3、关闭线程池
![Alt text](./1560236210057.png)


#####9.2.4、合理地配置线程池
![Alt text](./1560236370525.png)

	建议使用有界队列

#####9.2.5、线程池的监控
![Alt text](./1560236455535.png)
...
#####10.1.1、Executor框架的两级调度模型
	1、Executor框架结构
![Alt text](./1560238965164.png)
![Alt text](./1560239077917.png)
![Alt text](./1560239116275.png)
![Alt text](./1560239206297.png)

	2、Executor框架成员
	主要成员：ThreadPoolExecutor、ScheduledThreadPoolExecutor、Future接口、Runnable接口、Callable接口、Executors
	1、ThreadPoolExecutor
![Alt text](./1560239468235.png)
	
	2、ScheduledThreadPoolExecutor

	3、Future接口
![Alt text](./1560239604340.png)

	4、Runnable接口和Callable接口
![Alt text](./1560239648537.png)

####10.2、ThreadPoolExecutor详解
![Alt text](./1560239735269.png)
#####10.2.1、FixedThreadPool详解
	可重用固定线程数的线程池
![Alt text](./1560239897717.png)
#####10.2.2、SingleThreadExecutor详解
	使用单个worker线程的Executor
![Alt text](./1560240010163.png)

#####10.2.3、CachedThreadPool详解
	会根据需要创建新的线程的线程池
![Alt text](./1560240070483.png)
